...
I'm an MCB master's student in Grenoble, first year, soon to be on the Neuroscience & Neurobiology track.
...
Outside of coursework I build tools from scratch, mostly bioinformatics-adjacent. Not because the
libraries don't exist — because reimplementing something yourself is the fastest way to actually
understand it, instead of just knowing which function to call. When I'm not doing that, I'm
usually playing electric guitar, which is also the reason [TabEngine](https://github.com/Scrimas/TabEngine)
exists.
...
The most polished thing I've shipped so far. A free, open-source desktop guitar tab player for
Guitar Pro files (GP3/4/5/GPX) — no account, no ads, no premium wall. I built it because Songsterr
kept moving free features behind a paywall. It's licensed GPLv3 specifically so it can't be forked
and resold: free now, free forever.
